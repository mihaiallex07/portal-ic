// ============================================================
// Biblioteca Module — Portal Inginerie Creativă
// Afișează conținut din Google Drive folder 04. Biblioteca tehnica
...
const Biblioteca = {
  ...
  render() {
    ...
    container.innerHTML = `
      <div style="margin-bottom:20px">
        <h2 style="font-size:22px;font-weight:700;margin:0 0 4px">Bibliotecă Tehnică</h2>
        <p style="color:var(--text-muted);font-size:13px;margin:0">Standarde, normative și detalii tehnice</p>
      </div>
      <div style="display:flex;gap:16px;height:calc(100vh - 180px);min-height:500px">
        <div id="biblioteca-sidebar" style="width:280px;flex-shrink:0;background:var(--card-bg);border:1px solid var(--border);border-radius:10px;overflow-y:auto;padding:8px">
          <div id="biblioteca-breadcrumb" style="padding:8px 12px 4px;font-size:11px;font-weight:600;color:var(--text-muted);text-transform:uppercase;letter-spacing:0.5px">Foldere</div>
          <div id="biblioteca-file-list">
        ...
        <div id="biblioteca-viewer" style="flex:1;background:var(--card-bg);border:1px solid var(--border);border-radius:10px;overflow:hidden;display:flex;align-items:center;justify-content:center">
  ...
  async loadFolder(folderId) {
    const listEl = document.getElementById('biblioteca-file-list');
    const breadcrumb = document.getElementById('biblioteca-breadcrumb');
    if (!listEl) return;
    ...
    // Breadcrumb
    if (breadcrumb) {
      const crumbs = [{ id: this.FOLDER_ID, name: 'Bibliotecă' }, ...this.currentPath];
      breadcrumb.innerHTML = crumbs.map((c, i) => {
        const isLast = i === crumbs.length - 1;
        return isLast
          ? `<span style="color:var(--text);font-weight:600">${c.name}</span>`
          : `<button onclick="Biblioteca.navigateTo(${i})" style="border:none;background:none;cursor:pointer;color:var(--primary);font-size:11px;padding:0">${c.name}</button><span style="color:var(--text-muted)"> / </span>`;
      }).join('');
    }
    ...
    try {
      ...
      if (files === null) {
        ...
        listEl.innerHTML = `
          <div style="padding:16px;text-align:center">
            ...
            <button onclick="Biblioteca.connectDrive()"
      ...
      // Foldere primele, apoi fișiere
      const folders = files.filter(f => f.mimeType === 'application/vnd.google-apps.folder');
      const docs = files.filter(f => f.mimeType !== 'application/vnd.google-apps.folder');

      listEl.innerHTML = [
        ...folders.map(f => `
          <button onclick="Biblioteca.enterFolder('${f.id}','${f.name.replace(/'/g, "\\'")}'))"
        ...
        `),
        ...docs.map(f => `
          <button onclick="Biblioteca.openFile('${f.id}','${f.mimeType}')"
            style="display:flex;align-items:center;gap:8px;width:100%;padding:8px 12px;border:none;background:none;cursor:pointer;border-radius:6px;text-align:left;transition:background 0.15s"
            onmouseover="this.style.background='var(--bg-secondary)'" onmouseout="this.style.background='none'"
            id="file-btn-${f.id}">
      ...
      const listEl2 = document.getElementById('biblioteca-file-list');
  ...
  openFile(fileId, mimeType) {
    ...
    DriveViewer.showInViewer('biblioteca-viewer', fileId, mimeType);
  }
};
