      ...
      userGroups.forEach(group => {
        ...
        group.users.forEach(user => {
          ...
          const leaveMarkers = this.layoutLeaveMarkers(user.id, startDate, days).map(segment => `
            <div title="Concediu aprobat" style="position:absolute;left:${segment.left}px;top:3px;width:${segment.width}px;height:calc(100% - 6px);box-sizing:border-box;pointer-events:none;z-index:3;display:flex;align-items:center;justify-content:center;color:#fff;font-size:11px;font-weight:900;line-height:1;background:repeating-linear-gradient(135deg,#7C3AED 0,#7C3AED 7px,#6D28D9 7px,#6D28D9 14px);border:1px solid rgba(76,29,149,.9);border-radius:4px;box-shadow:inset 0 0 0 1px rgba(255,255,255,.12)">C</div>
          `).join('');
